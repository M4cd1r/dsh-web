...
- **个人套餐页签**：每个已配置且暴露套餐端点的 provider 的配额窗口——已用百分比与重置时间（Kimi For Coding 5 小时/每周、GLM 编程计划 5 小时/每周（旧版 5 小时、现行与积分制套餐同样支持）、OpenCode Go 滚动/每周/每月、MiniMax 5 小时/每周、Codex / ChatGPT 订阅 5 小时/每周）。没有真实套餐/订阅体系的厂商（DeepSeek、ZenMux、Moonshot、OpenRouter、SiliconFlow）不出现在此页签，其余额显示在用量页签。
...
支持的余额端点：DeepSeek（官方运行时路由 `deepseek-official` 与目录别名 `deepseek` 均可解析）、Moonshot（国内/国际）、OpenRouter、SiliconFlow（国内/国际）、ZenMux。支持的套餐端点：Kimi For Coding、GLM 编程计划（国际 `zai` 路由及其 `zai-coding` 旧别名走 api.z.ai，国内 `zai-coding-cn` 路由走 open.bigmodel.cn；旧版仅 5 小时、现行 5 小时 + 每周与积分制套餐均由同一配额端点解析——见 [z.ai 用量政策](https://docs.z.ai/devpack/usage-policy)）、OpenCode Go、MiniMax、Codex / ChatGPT 订阅（OAuth access token 取自 pi-ai grant；token 过期时显示错误行，宿主下次跑 Codex 请求自动刷新后恢复）。没有程序化端点的 provider（Qwen token 套餐、OpenCode Zen 按量、Anthropic、OpenAI）仅列出，不展示数据。
...
- GLM 编程计划的 MCP/请求上限（`TIME_LIMIT` 条目）不展示：仅渲染 token/积分配额窗口（5 小时/每周）。
