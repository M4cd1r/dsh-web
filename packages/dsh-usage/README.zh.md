...
- **侧边栏面板**：位于侧边栏插件区的可折叠用量面板（在技能中心入口与工作区浏览器之间），默认展开，折叠状态按浏览器记忆。面板列出宿主已知的每个 provider：套餐类展示 5 小时 / 每周 / 每月配额进度条（每窗口的已用百分比与重置时间），仅有余额的展示「名称 + 剩余金额」行（如 DeepSeek ¥12.34），无程序化端点的一行灰字说明。展开且页面可见时每 60 秒重读同一份 overview，并提供手动探测刷新；设置行可关闭该面板（`sidebarPanel`）。
...
- 探测完全在宿主侧按轮询周期执行（默认 60 秒，可手动刷新）；API key 经宿主凭据缝解析（`llm-pi-ai` 记录、`apiKeyEnv` 引用），永不进入浏览器。overview 与 refresh 路由接受 loopback 本机请求，以及安装 `dsh-remote-web-ui` 后的有效配对设备请求；其他远程请求仍返回 `403`。
...
| `sidebarPanel` | `true` | 在侧边栏插件区放置可折叠用量面板（关闭后侧边栏入口消失，设置分区不受影响） |
