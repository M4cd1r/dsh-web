...
- **Plans tab (个人套餐)**: coding-plan quota windows for every configured provider that exposes one — used percent and reset time per window (Kimi For Coding 5h/week, GLM Coding Plan 5h/week on legacy, current and credits-based plans alike, OpenCode Go rolling/weekly/monthly, MiniMax 5h/week, Codex / ChatGPT subscription 5h/week). Providers without a real plan or subscription (DeepSeek, ZenMux, Moonshot, OpenRouter, SiliconFlow) never appear on this tab; their balance shows on the usage tab instead.
...
Supported balance endpoints: DeepSeek (the official live route `deepseek-official` and the catalog alias `deepseek` both resolve), Moonshot (CN/international), OpenRouter, SiliconFlow (CN/international), ZenMux. Supported plan endpoints: Kimi For Coding, GLM Coding Plan (the international `zai` route and its `zai-coding` alias via api.z.ai, the mainland `zai-coding-cn` route via open.bigmodel.cn; legacy 5-hour-only plans, current 5h + weekly plans and credits-based plans are all parsed from the same quota endpoint — see the [z.ai usage policy](https://docs.z.ai/devpack/usage-policy)), OpenCode Go, MiniMax, Codex / ChatGPT subscription (OAuth access token from the pi-ai grant; a stale token shows an error until the harness next refreshes it). Providers without a programmatic endpoint (Qwen token plans, OpenCode Zen PAYG, Anthropic, OpenAI) are listed without facts.
...
- GLM Coding Plan MCP/request ceilings (`TIME_LIMIT` entries) are not rendered: only the token/credit quota windows (5h/week) are shown.
