...
- **Sidebar panel**: a collapsible usage panel seated in the sidebar family (between the Skill center entry and the Workspaces browser), expanded by default with the collapse state remembered per browser. It lists every provider the host knows: coding-plan providers as 5-hour / weekly / monthly quota progress bars (used percent and reset time per window), balance-only providers as a name-and-remaining row (for example DeepSeek ¥12.34), and providers with no programmatic endpoint as a muted note. While expanded and visible it re-reads the same overview every 60 s and offers a manual probe refresh; the section's settings row switches the panel off (`sidebarPanel`).
...
- Probes run entirely host-side on a poll cycle (default 60 s, manual refresh button); API keys are resolved through the harness credential seam (`llm-pi-ai` records, `apiKeyEnv` references) and never reach the browser. The overview and refresh routes accept loopback hosts plus live paired-device requests when `dsh-remote-web-ui` is installed; all other remote requests still receive `403`.
...
| `sidebarPanel` | `true` | Seat the collapsible usage panel in the sidebar family (off = the sidebar entry disappears; the settings section is unaffected) |
