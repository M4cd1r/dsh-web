...
| `sidebar-panel` | usage | 侧边栏用量面板根（可折叠；入口行为家族注入的侧栏行）；`[data-dsh-plugin="usage"][data-dsh-part="sidebar-panel"]`，入口行锚定 `[data-dsh-usage-entry]` |
...
| `usage` | dsh-usage | 一级设置分区 settings.section id `dsh-usage`（创意工坊下方）；`[data-dsh-plugin="usage"]`；侧边栏入口行 `[data-dsh-usage-entry]`（面板根 `[data-dsh-part="sidebar-panel"]`，家族区内技能中心入口之后） |
